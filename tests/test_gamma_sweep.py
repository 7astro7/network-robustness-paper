...
# ---------------------------------------------------------------------------
# GammaSweepExperiment._detect_positive_drift
# ---------------------------------------------------------------------------

class TestDetectPositiveDrift:
    @pytest.fixture
    def sweep(self):
        return GammaSweepExperiment(n=100, gammas=[2.5], seeds=[0])

    def test_flat_signal_returns_nan(self, sweep):
        qs = np.linspace(0, 0.9, 50)
        signal = np.ones(49)
        result = sweep._detect_positive_drift(qs, signal, m=3, q0=0.15)
        assert np.isnan(result)

    def test_monotone_increase_triggers(self, sweep):
        qs = np.linspace(0, 0.9, 50)
        signal = np.linspace(0, 1, 49)
        result = sweep._detect_positive_drift(qs, signal, m=3, q0=0.15)
        assert np.isfinite(result)
        assert result >= 0.15

    def test_decreasing_signal_returns_nan(self, sweep):
        qs = np.linspace(0, 0.9, 50)
        signal = np.linspace(1, 0, 49)
        result = sweep._detect_positive_drift(qs, signal, m=3, q0=0.15)
        assert np.isnan(result)

    def test_one_violation_still_triggers(self, sweep):
        """max_violations=1 means one non-increase in window is tolerated."""
        qs = np.linspace(0, 0.9, 50)
        signal = np.linspace(0, 1, 49)
        qs_mid = 0.5 * (qs[:-1] + qs[1:])
        start = int(np.searchsorted(qs_mid, 0.15, side="left"))
        signal[start + 1] = signal[start]
        result = sweep._detect_positive_drift(qs, signal, m=3, q0=0.15, max_violations=1)
        assert np.isfinite(result)

    def test_two_violations_returns_nan_with_max_violations_one(self, sweep):
        qs = np.linspace(0, 0.9, 50)
        signal = np.zeros(49)
        qs_mid = 0.5 * (qs[:-1] + qs[1:])
        start = int(np.searchsorted(qs_mid, 0.15, side="left"))
        signal[start:start + 4] = [1.0, 0.9, 0.8, 2.0]
        result = sweep._detect_positive_drift(qs, signal, m=3, q0=0.15, max_violations=1)
        assert np.isnan(result)

    def test_start_beyond_signal_returns_nan(self, sweep):
        """If q0 is beyond all midpoints, should return nan gracefully."""
        qs = np.linspace(0, 0.5, 10)
        signal = np.linspace(0, 1, 9)
        result = sweep._detect_positive_drift(qs, signal, m=3, q0=0.9)
        assert np.isnan(result)

    def test_signal_length_mismatch_raises(self, sweep):
        qs = np.linspace(0, 0.9, 50)
        signal = np.ones(30)
        with pytest.raises(ValueError):
            sweep._detect_positive_drift(qs, signal)

    def test_min_net_increase_filters_small_drift(self, sweep):
        qs = np.linspace(0, 0.9, 50)
        signal = np.zeros(49)
        qs_mid = 0.5 * (qs[:-1] + qs[1:])
        start = int(np.searchsorted(qs_mid, 0.15, side="left"))
        signal[start:start + 4] = [0.0, 0.001, 0.002, 0.003]
        result = sweep._detect_positive_drift(
            qs, signal, m=3, q0=0.15, max_violations=0, min_net_increase=0.1
        )
        assert np.isnan(result)
